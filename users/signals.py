...
def create_follower(sender, instance, created, **kwargs):
    talabatgramMainUser = User.objects.get(id=1)
    if created:
        Follower.objects.create(follower=instance, being_followed=instance)
        if not instance.pk == 1:
            Follower.objects.create(follower=instance, being_followed=talabatgramMainUser)
            Follower.objects.create(follower=talabatgramMainUser, being_followed=instance)
...
def create_objects(sender, instance, created, **kwargs):
    ...
    if created:
        Message.objects.create(sender=talabatgramMainUser, receiver=instance, content='This is your inbox! Click me to view our conversation.')
        ...
        Comment.objects.create(post=post, author=talabatgramMainUser, content='Here is your first comment!')
